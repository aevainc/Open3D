            ...
            if (key_to_id_map.find(query_keys[j]) != key_to_id_map.end()) {
                if (key_to_id_map[query_keys[j]] != query_vals[j]) {
                    ...
                    fprintf(stderr,
                            ...
                            query_keys[j], key_to_id_map[query_keys[j]],
                            query_vals[j]);
#endif
                    errors++;
                }
            ...
            if (id_to_key_map.find(query_vals[j]) != id_to_key_map.end()) {
                if (id_to_key_map[query_vals[j]] != query_keys[j]) {
                    ...
                    fprintf(stderr,
                            ...
                            query_vals[j], id_to_key_map[query_vals[j]],
                            query_keys[j]);
#endif
                    errors++;
                }
    ...
    for (unsigned iteration = 0; iteration < kMaxIterations; ++iteration) {
        ...
        for (unsigned multiplicity = 1; multiplicity <= multiplicity_max;
            ...
            switch (htt) {
                ...
                case CUDPP_MULTIVALUE_HASH_TABLE:
                    for (unsigned i = 0; i < kInputSize; ++i) {
                        pairs_multivalue[input_keys[i]].push_back(
                                input_vals[i]);
                    }
                    for (std::map<unsigned, std::vector<unsigned>>::iterator
                                 itr = pairs_multivalue.begin();
            ...
            for (unsigned i = 0; i < kNumSpaceUsagesToTest; ++i) {
                ...
                for (unsigned failure = 0; failure <= failure_trials;
                     ++failure) {
                    ...
                    switch (htt) {
                        ...
                        case CUDPP_COMPACTING_HASH_TABLE:
                            ...
                            break;
                        case CUDPP_MULTIVALUE_HASH_TABLE:
                            result = cudppHashRetrieve(
                                    hash_table_handle, d_test_keys,
                    ...
                    switch (htt) {
                        case CUDPP_BASIC_HASH_TABLE:
                            CUDA_SAFE_CALL(
                                    cudaMemcpy(query_vals, d_test_vals,
                                               sizeof(unsigned) * kInputSize,
                                               cudaMemcpyDeviceToHost));
                            errors +=
                                    CheckResults_basic(kInputSize, pairs_basic,
                        ...
                        case CUDPP_COMPACTING_HASH_TABLE:
                            CUDA_SAFE_CALL(
                                    cudaMemcpy(query_vals, d_test_vals,
                                               sizeof(unsigned) * kInputSize,
                                               cudaMemcpyDeviceToHost));
                            errors += CheckResults_compacting(
                                    kInputSize, pairs_compacting, query_keys,
                                    query_vals);
                            break;
                        case CUDPP_MULTIVALUE_HASH_TABLE:
                            CUDA_SAFE_CALL(
                                    cudaMemcpy(query_vals_multivalue,
                                               d_test_vals_multivalue,
                                               sizeof(uint2) * kInputSize,
                                               cudaMemcpyDeviceToHost));
                            errors += CheckResults_multivalue(
                                    kInputSize, pairs_multivalue, sorted_values,
                                    query_keys, query_vals_multivalue);
                            break;
                        default:
                            errors++;
                            fprintf(stderr,
    ...
    for (CUDPPHashTableType htt = CUDPP_BASIC_HASH_TABLE;
        ...
        if (runAll || ((htt == CUDPP_BASIC_HASH_TABLE) && runBasicHash) ||
            ...
            total_errors += testHashTable(
                    theCudpp, htt, kMaxIterations, kInputSize, number_pool,
                    pool_size, input_vals, input_keys, d_test_vals,
                    d_test_vals_multivalue, d_test_keys, query_vals,
                    query_vals_multivalue, query_keys, skipEveryNTests,
                    skipHighMultiplicities, testNumber);
        }
    }
    if (total_errors == 0) {
